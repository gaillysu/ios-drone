...
class LocationManager: NSObject {
    ...
    fileprivate override init() {
        ...
        if CLLocationManager.headingAvailable() {
            ...
            let banner:MEDBanner = MEDBanner(title: "GPS use of infor", subtitle: "GPS devices do not available", image: nil, backgroundColor: UIColor.red, didTapBlock: nil)
    ...
    func startLocation() {
        if CLLocationManager.locationServicesEnabled() {
            ...
            let banner:MEDBanner = MEDBanner(title: "Location services is not open", subtitle: nil, image: nil, backgroundColor: UIColor.red, didTapBlock: nil)
            banner.show(duration:1.2)
        }
    }
