...
open class MenuItemView: UIView {
    ...
    fileprivate func layoutImageView() {
        ...
        let horizontalConstraints = NSLayoutConstraint.constraints(withVisualFormat: "H:|[imageView]|", options: [], metrics: nil, views: viewsDictionary)
        let verticalConstraints = NSLayoutConstraint.constraints(withVisualFormat: "V:|[imageView]|", options: [], metrics: nil, views: viewsDictionary)
        NSLayoutConstraint.activate(horizontalConstraints + verticalConstraints)
        
        let width: CGFloat
        switch options.menuDisplayMode {
        case .segmentedControl: width = UIApplication.shared.keyWindow!.bounds.size.width / CGFloat(options.menuItemCount)
